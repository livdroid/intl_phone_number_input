...
class InternationalPhoneNumberInput extends StatefulWidget {
  ...
  final bool keepCountriesGivenOrder;
  ...
  InternationalPhoneNumberInput({
    ...
    this.autofillHints,
    this.countries,
    this.keepCountriesGivenOrder = true,
  }) : super(key: key);
...
class _InputWidgetState extends State<InternationalPhoneNumberInput> {
  ...
  void loadCountries({Country? previouslySelectedCountry}) {
    if (this.mounted) {
      ...
      if (countryComparator != null && !widget.keepCountriesGivenOrder) {
        countries.sort(countryComparator);
      }

      if (widget.keepCountriesGivenOrder) {
        /// Keep user country order
        final List<Country> userCountries = [];
        final List<String> userCountryCodes = widget.countries ?? [];
        for (final country in userCountryCodes) {
            final Country? userCountry = countries.firstWhereOrNull(
            (element) => element.alpha2Code == country,
            );
          if (userCountry != null) userCountries.add(userCountry);
        }
